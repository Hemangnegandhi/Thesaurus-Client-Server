...
taken from github Developed by
...
public class Server 
{
	private int port = 1235; //Declaration of global variables for use
	 ...
	 */
	private class SocketHandler extends Thread /* Socket handling section in the server */
	{
		private Socket clientSocket;
		...
		public synchronized void run()
		{
			try
			{
				...
				while(true){
					...
					{
						String synonmys1 = new ReadFile().getSynonyms(selectedWord.trim());
						/* Involving the class that finds out the syomys from the text file that is the database*/
						
						
						
						
						
						if (synonmys1!=null && synonmys1!=selectedWord)
						{
							userGUIOutput=new StringBuilder(synonmys1+"");
						}
						else
						{
							userGUIOutput=new StringBuilder("No synoym found");
						}
					}	
			...
			finally
			{
				...
				catch(IOException e) /*Exception and error handling */
				{
					serverMonitorLog.append("\n"+e.getMessage());
				}
			}
